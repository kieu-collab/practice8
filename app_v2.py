...
# Streamlit Community Cloud has limited shared CPU.
# A small delay keeps RoBERTa inference from saturating the CPU continuously.
...
# ------------------------------------------------------------------------------
# 4. HỆ THỐNG PRODUCER & XỬ LÝ STREAMING (TỐI ƯU CHO STREAMLIT CLOUD)
# ------------------------------------------------------------------------------
    ...
    try:
        ...
        with gzip.GzipFile(fileobj=response.raw) as gz:
            for line in gz:
                ...
                # Giảm tải CPU trên Streamlit Community Cloud.
                # Vẫn giữ đúng bản chất streaming nhưng không chạy RoBERTa 100% CPU liên tục.
...
# ------------------------------------------------------------------------------
# 7. HÀM CHẠY STREAMING (TỐI ƯU TÀI NGUYÊN CLOUD)
# ------------------------------------------------------------------------------
def run_stream_demo(duration_seconds=DURATION_SECONDS):
    ...
    try:
        while time.monotonic() - started_at < duration_seconds:
            ...
            # Trên Streamlit Cloud, render Matplotlib quá dày sẽ gây CPU throttling.
            if current_time - last_render >= DASHBOARD_REFRESH_SECONDS:
...
with st.sidebar:
    ...
    st.caption(
        "Khuyến nghị test 60–120 giây trước. Chạy 1800 giây với RoBERTa trên "
        "Streamlit Community Cloud có thể bị giới hạn CPU."
    )
